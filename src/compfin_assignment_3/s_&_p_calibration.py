...
#define a function to determine the LSM between the model and market volatilities
# def loss_function(params, S0, r, strikes, maturities, interp_vols):
#     """
#     Loss function to compute the difference between model and market volatilities.
#     """
#     # Unpack parameters
...
#             # Unpack the strikes and maturities
#             K = strikes[i]
#             T = maturities[j]
#             market_iv = interp_vols[j, i]  # i: maturity, j: strike

#     # Compute model volatilities using Heston model
#             heston_price = heston_call_price(S0, K, r, T, v0, kappa, theta, sigma, rho)
#             model_iv = implied_volatility(heston_price, S0, K, T, r)	

#     # Compute the loss as the sum of squared differences
#     # between model and market volatilities
#             if not np.isnan(market_iv): 
#                 loss += np.sum((model_iv - market_iv) ** 2)
#     return loss
...
#     # results_by_date[date] = result
#     # iv_surfaces_by_date[date] = interp_vols
#     strikes = strikes[::5]         # 100 → 20
#     maturities = maturities[::3]   # 15 → 5
#     interp_vols = interp_vols[::3, ::5]
...
#             print(f"[Iter {iteration_counter}] Loss = {loss:.4f}, Count = {count}")
...
#                 except:
#                     continue
#         return 1e6  if count == 0 else loss / count
...
#         bounds=bounds,
#         method='L-BFGS-B',
#         options = {"maxfun": 300, "disp": True}
#     )

#     print(f"Calibration result for {date}: {result.fun:.4f}")
#     print(f"Implied Volatility Surface for {date}:")
#     print(result)

#     fitted_ivols = np.zeros_like(interp_vols)

#     for j, T in enumerate(maturities):
#         for i, K in enumerate(strikes):
#             try:
#                 price = heston_call_price(S0, K, r, T, *result.x)
#                 if price > 0:
#                     fitted_ivols[j, i] = implied_volatility(price, S0, K, T, r)
#                 else:
#                     fitted_ivols[j, i] = np.nan
#             except:
#                 fitted_ivols[j, i] = np.nan

#     results_by_date[date] = result
#     iv_surfaces_by_date[date] = {
#         "fitted": fitted_ivols,
#         "market": interp_vols,
#         "strikes": strikes,
...
#         strikes=strikes,
#         maturities=maturities,
#         market_iv=interp_vols,
#         model_iv=fitted_ivols,
#         date_str=date
#     )

#     valid_mask = np.isfinite(interp_vols) & np.isfinite(fitted_ivols)
#     errors = fitted_ivols[valid_mask] - interp_vols[valid_mask]

#     rmse = np.sqrt(np.mean(errors**2))
#     mae = np.mean(np.abs(errors))
#     max_error = np.max(np.abs(errors))

#     print(f"Error metrics for {date}:")
